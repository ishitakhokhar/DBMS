...
SELECT REGION,SUM(SALES_AMOUNT) FROM SALES_DATA WHERE YEAR=2023 GROUP BY REGION ORDER BY SUM(SALES_AMOUNT) DESC
...
SELECT PRODUCT,AVG(SALES_AMOUNT) FROM SALES_DATA WHERE SALES_AMOUNT BETWEEN 1000 AND 2000 GROUP BY PRODUCT ORDER BY PRODUCT
--2
...
SELECT REGION,COUNT(DISTINCT PRODUCT) FROM SALES_DATA GROUP BY REGION HAVING (COUNT(PRODUCT)-COUNT(DISTINCT PRODUCT))>=1
--5
